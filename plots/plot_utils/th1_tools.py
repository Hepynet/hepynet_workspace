...
class HistCollection(object):
    ...
    def draw(
        ...
        auto_xrange_method="default",
        auto_yrange_method="default",
    ) -> None:
        ...
        Args:
            ...
            norm_factor: If draw_norm is True, histograms will be normalized to 
                this value.
            auto_xrange_method: can be None, default or major
            auto_yrange_method: can be None, default
        ...
        x_min_use = math.inf
        x_max_use = -math.inf
        x_min_major = math.inf
        x_max_major = -math.inf
        for hist in self._hist_list:
            ...
            if x_max > x_max_use:
                x_max_use = x_max
            # find x range which contains about most of the events
            total_weight = hist.get_hist().Integral("width")
            filter_threshold = total_weight * 0.01 / hist.get_hist().GetBinWidth(1)
            x_min = hist.get_hist().FindFirstBinAbove(filter_threshold)
            x_max = hist.get_hist().FindLastBinAbove(filter_threshold)
            if x_min < x_min_major:
                x_min_major = x_min
            if x_max > x_max_major:
                x_max_major = x_max
        ...
        #    x_max_use += 1
        if auto_xrange_method == "default":
            self._hist_list[0].get_hist().GetXaxis().SetRange(x_min_use, x_max_use)
        elif auto_xrange_method == "major":
            self._hist_list[0].get_hist().GetXaxis().SetRange(x_min_major, x_max_major)
        if auto_yrange_method == "default":
        ...
        ## >> hot fix
        if auto_xrange_method == "default":
            return (x_min_use, x_max_use)
        elif auto_xrange_method == "major":
            return (x_min_major, x_max_major)
        else:
            return (None, None)
...
class RatioPlot(object):
    ...
    def draw(self, draw_options: str = "e3", draw_err=True, draw_base_line=True):
        """Plots HistCollection on canvas.
        ...
        if draw_base_line:
            ...
            base_line.Draw("same")
        # plot ratio
        self._hist_ratio.set_config(self.style_cfg)
