...
df_dir = pathlib.Path(f"/data/zprime/data_frames/eb-check-charge-ordered-lep-pt")
...
def process_sample(sample_name, sample_path, is_sig, is_mc, m_truth_name):
    ...
    for chunk_pd in uproot.iterate(
        ...
        # additional_parameters mz1 - mz2
        mz1_mz2 = chunk_pd["mz1"].values - chunk_pd["mz2"].values
        chunk_pd["mz1_mz2"] = mz1_mz2
        # make pt ordered according to charge
        ptl1_list = list()
        ptl2_list = list()
        ptl3_list = list()
        ptl4_list = list()
        for i, row in chunk_pd.iterrows():
            lep1_charge = row["pidl1"]
            lep2_charge = row["pidl2"]
            lep3_charge = row["pidl3"]
            lep4_charge = row["pidl4"]
            lep1_pt = row["ptl1"]
            lep2_pt = row["ptl2"]
            lep3_pt = row["ptl3"]
            lep4_pt = row["ptl4"]
            if lep1_charge > 0:
                ptl1_list.append(lep1_pt)
                ptl2_list.append(lep2_pt)
                if lep2_charge > 0:
                    print(f"Warning! Lepton 1 and 2 have same sign!")
            else:
                ptl1_list.append(lep2_pt)
                ptl2_list.append(lep1_pt)
                if lep2_charge < 0:
                    print(f"Warning! Lepton 1 and 2 have same sign!")
            if lep3_charge > 0:
                ptl3_list.append(lep3_pt)
                ptl4_list.append(lep4_pt)
                if lep4_charge > 0:
                    print(f"Warning! Lepton 3 and 4 have same sign!")
            else:
                ptl3_list.append(lep4_pt)
                ptl4_list.append(lep3_pt)
                if lep4_charge < 0:
                    print(f"Warning! Lepton 1 and 2 have same sign!")
        chunk_pd = chunk_pd.assign(ptl1=ptl1_list)
        chunk_pd = chunk_pd.assign(ptl2=ptl2_list)
        chunk_pd = chunk_pd.assign(ptl3=ptl3_list)
        chunk_pd = chunk_pd.assign(ptl4=ptl4_list)
...
del low_df_list
save_path = low_dir / "zprime_low_m_4mu.feather"
...
del high_df_list
save_path = high_dir / "zprime_high_m_4mu.feather"
print(f"## Saving to {save_path}")
