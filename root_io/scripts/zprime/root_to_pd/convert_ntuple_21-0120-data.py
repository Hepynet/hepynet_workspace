...
feature_list = [
    ...
    "event",
    "quadtype",
    "weight",  # for data, weight is what needed, which is always 1
]
...
for var in variations:
    ...
    for data_name, data_file in data_names.items():
        ...
        low_df_list += process_sample (data_name, root_path, False, True, "mz2", var)
    ## dump
    low_df: pd.DataFrame = pd.concat(low_df_list, ignore_index=True)
    del low_df_list
    save_dir = low_dir / var
    save_dir.mkdir(parents=True, exist_ok=True)
    save_path = save_dir / "zprime_low_m_quadtype_2_data.feather"
    ...
    for data_name, data_file in data_names.items():
        root_path = ntup_dir / f"tree_{data_file}.root"
        high_df_list += process_sample (data_name, root_path, False, True, "mz1", var)
    ## dump
    high_df: pd.DataFrame = pd.concat(high_df_list, ignore_index=True)
    del high_df_list
    save_dir = high_dir / var
    save_dir.mkdir(parents=True, exist_ok=True)
    save_path = save_dir / "zprime_high_m_quadtype_2_data.feather"
    print(f"## Saving to {save_path}")
